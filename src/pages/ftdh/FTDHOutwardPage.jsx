...
export function FTDHOutwardPage() {
  ...
  const fetchCases = async () => {
    try {
      ...
      setError('Failed to load outward FTDH cases.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const outwardCases = useMemo(() => {
    return cases.map((c) => {
      const displayId = c.onelinkDisputeId || c.ftdhId || '—';
      return {
        id: c.id,
        fullDisputeId: displayId,
        disputeId: displayId.length > 16 ? displayId.slice(0, 16) + '...' : displayId,
        beneficiary: c.targetBank || '—',
        beneficiaryAccount: c.targetAccount || '—',
        sender: c.senderBank || '—',
        senderAccount: c.senderAccount || '—',
        trxDateTime: formatDisplayDateTime(c.transactionDate),
        stan: c.stan || '—',
        amount: formatCurrency(c.transactionAmount),
        status: c.ftdhType === 'ONUS' ? 'OnUs' : 'Layer',
      };
    });
  }, [cases]);
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-100px)] font-['Inter',sans-serif]">
      ...
      <div className="flex-1 min-h-0 rounded-[8px] border border-[#DAE1E7] bg-[#F9FAFB] overflow-hidden relative">
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center h-[calc(100%-64px)]">
            ...
            <p className="text-[16px] text-[#AFAFAF]">Loading outward cases...</p>
          </div>
        ) : error ? (
          <div className="flex flex-col items-center justify-center h-[calc(100%-64px)]">
            <p className="text-[16px] text-red-500 mb-2">{error}</p>
      ...
      />
      <FTDHOutwardSourceModal open={sourceModalOpen} onOpenChange={setSourceModalOpen} onCaseCreated={handleCaseCreated} />
      <FTDHOutwardLayeringModal open={layeringModalOpen} onOpenChange={setLayeringModalOpen} onCaseCreated={handleCaseCreated} />
    </div>
  );
}
